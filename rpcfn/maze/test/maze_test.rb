...
#####################################}
  ...
  @@maze1 = AMaze.new(MAZE1)
  @@maze2 = AMaze.new(MAZE2)
  @@maze3 = AMaze.new(MAZE3)
  def test_initialize()
    vertex_3_3 = @@maze1.at_location([3, 3])
    vertex_7_21 = @@maze1.at_location([7, 21])
    assert(vertex_3_3.neighbor?(DataStructures::Graph::Vertex.new([1, 3])))
    assert(vertex_7_21.neighbor?(DataStructures::Graph::Vertex.new([7, 23])))

    vertex_1_1 = @@maze2.at_location([1, 1])
    vertex_4_7 = @@maze2.at_location([4, 7])
    assert(vertex_1_1.neighbor?(DataStructures::Graph::Vertex.new([5, 1])))
    assert(vertex_4_7.neighbor?(DataStructures::Graph::Vertex.new([7, 7])))
  end
