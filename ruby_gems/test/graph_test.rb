...
class GraphTest < MiniTest::Unit::TestCase
  ...
  def test_initialize()
    @@graph.vertices.each_with_index { |vertex, index|
      case index
      when 0
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([5, 0])))
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([0, 5])))
        refute(vertex.neighbor?(DataStructures::Graph::Vertex.new([5, 7])))
      when 1
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([0, 0])))
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([7, 5])))
        refute(vertex.neighbor?(DataStructures::Graph::Vertex.new([5, 7])))
      when 2
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([0, 0])))
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([7, 0])))
        refute(vertex.neighbor?(DataStructures::Graph::Vertex.new([5, 7])))
      when 3
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([5, 0])))
        assert(vertex.neighbor?(DataStructures::Graph::Vertex.new([7, 5])))
